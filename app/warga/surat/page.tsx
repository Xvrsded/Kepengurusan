...
const STATUS_COLOR: Record<Letter["status"], string> = {
  ...
  Proses: "bg-orange-50 text-orange-600",
};
...
export default function WargaSuratPage() {
  ...
  const filteredLetters = useMemo(() => {
    return myLetters.filter((letter) => {
      const matchesSearch = `${letter.type} ${letter.date}`.toLowerCase().includes(search.toLowerCase());
      const matchesStatus = statusFilter === "Semua" ? true : letter.status === statusFilter;
      return matchesSearch && matchesStatus;
    });
  }, [myLetters, search, statusFilter]);
    ...
    <div className="min-h-screen bg-slate-50 font-sans max-w-md mx-auto relative shadow-2xl overflow-x-hidden pb-24">
      ...
      <div className="px-4 pt-4 flex flex-col gap-4 pb-3">
        <div className="grid grid-cols-2 gap-3 animate-in fade-in duration-500">
          <div className="bg-white rounded-4xl p-4 border border-slate-100 shadow-lg shadow-slate-100/70">
        ...
        <div className="bg-white rounded-4xl shadow-lg shadow-slate-100/70 overflow-hidden border border-slate-100 animate-in fade-in duration-500">
        ...
        <div className="bg-white rounded-[2rem] shadow-lg shadow-slate-100/70 overflow-hidden border border-slate-100 animate-in fade-in duration-500">
                ...
                <li key={letter.id} className="rounded-3xl border border-slate-100 bg-linear-to-br from-white to-slate-50/60 p-4 shadow-sm transition-all duration-300 hover:scale-[1.01] animate-in fade-in" style={{ animationDelay: `${index * 40}ms` }}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 min-w-0">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${letter.status === "Selesai" ? "bg-emerald-50 text-emerald-600" : "bg-orange-50 text-orange-600"}`}>
                        {letter.status === "Selesai" ? <CheckCircle2 size={20} /> : <Clock3 size={20} />}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-black text-slate-800 leading-snug">{letter.type}</p>
                        <p className="text-xs text-slate-500 mt-1">Diajukan pada {letter.date}</p>
                        <p className="text-xs text-slate-400 mt-2">Pemohon: {letter.applicant}</p>
                      </div>
                    </div>
